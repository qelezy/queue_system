...
- Модалка отчёта (параметры, предпросмотр таблицей, выгрузка CSV/XLSX/PDF): partial [Views/Reports/_ReportWorkflowModal.cshtml](Views/Reports/_ReportWorkflowModal.cshtml) (подключается из [Views/Reports/Index.cshtml](Views/Reports/Index.cshtml)), стили префикса `report-preview-modal__*` и `report-workflow-modal__*` в [wwwroot/css/additions.css](wwwroot/css/additions.css), скрипт [wwwroot/js/reports-index.js](wwwroot/js/reports-index.js) (классы `app-modal`, `is-open` / `is-preview`, закрытие как у других модалок). Требования к странице «Отчёты»: [reports-page-ui-requirements.md](reports-page-ui-requirements.md).
