...
echo === Dashboard test reseed (rollback + preflight + seed + verify) ===

sqlcmd -S "%SERVER%" -d "%DB%" -E -i dashboard-test-rollback.sql
if errorlevel 1 goto :fail

sqlcmd -S "%SERVER%" -d "%DB%" -E -i dashboard-test-preflight.sql
if errorlevel 1 goto :fail
...
echo.
echo Gotovo: 20 talonov na segodnya MSK, max ozhidanie do 20 min na /dashboard
exit /b 0
